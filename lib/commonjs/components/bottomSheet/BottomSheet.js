...
const BottomSheetComponent = /*#__PURE__*/(0, _react.forwardRef)(function BottomSheet(props, ref) {
  ...
  const animateToPosition = (0, _reactNativeReanimated.useWorkletCallback)(function animateToPosition(position, source, velocity = 0, configs) {
     ...
     */
    let offset = 0;
    if (animatedKeyboardState.value === _constants.KEYBOARD_STATE.SHOWN && keyboardBehavior !== _constants.KEYBOARD_BEHAVIOR.extend && position < animatedPosition.value) {
     ...
     */
    (0, _reactNativeReanimated.runOnJS)(handleOnAnimate)(animatedNextPositionIndex.value, position);
  ...
  }, [handleOnAnimate, keyboardBehavior, _providedAnimationConfigs, _providedOverrideReduceMotion]);
  /**
  ...
  const handleSnapToIndex = (0, _hooks.useStableCallback)(function handleSnapToIndex(index, animationConfigs) {
    ...
    if (!(index >= -1 && (index <= snapPoints.length - 1 || Array.isArray(_providedSnapPoints) && index <= _providedSnapPoints?.length - 1))) {
      if (__DEV__) {
        console.warn(`[BottomSheet] 'index' was provided but out of the provided snap points range! expected value to be between -1, ${snapPoints.length - 1}`);
      }
      return;
    }
    if (__DEV__) {
